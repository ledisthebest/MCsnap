(function (wHandle, wjQuery) {
     ...
     *
     */
    var userScoreGlobal = 0;
    var playerSmallestCellScore = 0;
    var ignoreSmall = false;
    var cellTrans = false;
    var freezeCell = false;
    var posX = 0;
    var posY = 0;
    var lastPlayerKiller = null;
    ...
    function gameLoop() {
        ...
        wHandle.onkeydown = function (event) {
            switch (event.keyCode) {
                ...
                case 13:
                    if (isTyping) {
                        if (lastChatSendedTime > 0 && lastChatSendedTime > new Date().getTime() - 1000 * 3) {
                            return false;
                        }

                        lastChatSendedTime = new Date().getTime();
    ...
    function showOverlays(arg) {
        wjQuery("#overlays").css('visibility', 'visible');
        wjQuery("#overlays").show();
    ...
    function drawChatBoard() {
        ...
        for (var i = 0; i < (len - from); i++) {
            ...
            chatName.font = 'Bold 90px Sans-Serif';
            ...
            chatText.font = 'Bold 90px Sans-Serif';
    ...
    function updateNodes(view, offset) {
        ...
        for (i = 0; i < queueLength; ++i) {
            ...
            if (killer && killedNode) {
                ...
                if (killer.name && killedNode.name) {
                    if (killedNode.name == userNickName && killedNode.color == playerColor) {
                        lastPlayerKiller = {'name': killer.name, 'color': killer.color};
                    }
                }
            }
        }
        ...
        if (playerCells.length) {
            playBtnJustClicked = false;
        }

        if (ua === true) {
            if (playerCells.length === 0) {
                if (hasOverlay == false) {
                    if (playBtnJustClicked == false) {
                        if (isSpectating == false) {
                            ...
                            playerColor = null;
                            playerSmallestCellScore = 0;
                            userScoreGlobal = 0;

                            if (lastPlayerKiller) {
                                var txt = '［SYSTEM］［1］' + JSON.stringify({
                                        'name': lastPlayerKiller.name,
                                        'color': lastPlayerKiller.color
                                    });
                                lastPlayerKiller = null;
    ...
    function trySendNickName() {
        if (playBtnJustClicked) {
            sendNickName();
        }
    ...
    function canvasResize() {
        window.scrollTo(0, 0);
        canvasWidth = wjQuery(wHandle).innerWidth();
        canvasHeight = wjQuery(wHandle).innerHeight();
    ...
    function drawGameScene() {
        ...
        if (0 != userScore) {
            ...
            scoreText.setValue(wHandle.txt['score'] + ' : ' + ~~(userScore / 100));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = .2;
    ...
    function calcUserScore() {
        var playerSmallestCellScore2 = 0;
        for (var score = 0, i = 0; i < playerCells.length; i++) {
            if (playerSmallestCellScore2 == 0) {
                playerSmallestCellScore2 = playerCells[i].size;
            }
            else if (playerSmallestCellScore2 > playerCells[i].size) {
                playerSmallestCellScore2 = playerCells[i].size;
            }
        ...
        playerSmallestCellScore = playerSmallestCellScore2;

        userScoreGlobal = score;
    ...
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 != leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                var scaleFactor = Math.min(0.22 * canvasHeight, Math.min(widthOrigin, .3 * canvasWidth)) / widthOrigin;
                lbCanvas.width = widthOrigin * scaleFactor;
                ...
                ctx.strokeStyle = '#000000';
                ctx.fillStyle = 'rgb(22, 32, 56)';
                ctx.lineWidth = 2;
                roundedOne(ctx, 0, 0, widthOrigin, boardLength, 18, true, true);
                ctx.fillStyle = '#EF7845';
                var c = wHandle.txt['leaderboard'];
                ctx.font = '30px Ubuntu';
                ...
                ctx.shadowOffsetY = 2;
                ctx.shadowBlur = 2;
                ctx.fillText(c, widthOrigin / 2 - ctx.measureText(c).width / 2, 40);
                ...
                if (null == teamScores) {
                    for (ctx.font = "20px Ubuntu", b = 0; b < leaderBoard.length; ++b) {
                        c = leaderBoard[b].name || wHandle.txt['noNamePlayerName'];
                        ;
                        if (!showName) {
                            (c = wHandle.txt['noNamePlayerName']);
                        }
    ...
    wHandle.setCellTrans = function (arg) {
        cellTrans = arg;
    };
    wHandle.setIgnoreSmall = function (arg) {
        ignoreSmall = arg;
    };
    ...
    Cell.prototype = {
        ...
        drawOneCell: function (ctx) {
            if (true) {
                if (ignoreSmall && this.isVirus == false) {
                    if (this.size == 39) {
                        if (playerSmallestCellScore > 600 || userScoreGlobal > 100000) {
                            return false;
                        }
                    }
                }
    ...
    function roundedOne(ctx, x, y, width, height, radius, fill, stroke) {
    ...
    wHandle.gameLoop = gameLoop;
    wHandle.showOverlays = showOverlays;

    var isSystemMsg = function (msg) {
        var checkWord = '［SYSTEM］［';

        if (msg.indexOf(checkWord) === 0) {
            return true;
        }
    ...
    var systemMsg1MessageInfs = ['학살중입니다.', '미쳐 날뛰고 있습니다.', '도저히 막을 수 없습니다.', '전장의 지배자', '전장의 화신', '전설의 출현'];

    var systemMsg1Infs = {};

    var getChatMsg = function (msg) {
        msg = wjQuery.trim(msg);

        if (!msg) {
            return '';
        }

        if (isSystemMsg(msg) == false) {
            return msg;
        }
        ...
        if (msg.indexOf(checkWord) === 0) {
            code = msg.replace(checkWord, '').split(checkWord2)[0];
            body = msg.replace(checkWord + code + checkWord2, '');
            body = JSON.parse(body);
        }

        if (code == 1) {
            if (playerColor != body.color || userNickName != body.name) {
                code = null;
            }
        }

        if (code == 1) {
            if (typeof systemMsg1Infs[body.name + body.color] == 'undefined') {
                systemMsg1Infs[body.name + body.color] = {
                    'point': 0,
                    'time': 0
                };
            }

            if (systemMsg1Infs[body.name + body.color].time + 60 * 60 * 24 * 1000 > new Date().getTime()) {
                systemMsg1Infs[body.name + body.color].point++;
            }
            else {
                systemMsg1Infs[body.name + body.color].point = 1;
            }

            systemMsg1Infs[body.name + body.color].time = new Date().getTime();

            if (typeof systemMsg1MessageInfs[systemMsg1Infs[body.name + body.color].point - 1] != 'undefined') {
                msg2 = systemMsg1MessageInfs[systemMsg1Infs[body.name + body.color].point - 1];
            }
            else if (systemMsg1Infs[body.name + body.color].point > systemMsg1MessageInfs.length) {
                msg2 = systemMsg1MessageInfs[systemMsg1MessageInfs.length - 1] + '(' + systemMsg1Infs[body.name + body.color].point + '킬)';
            }
    ...
    // 개발에 필요해서 밖으로 꺼냄
    wHandle.playerColor = playerColor;
